...
drawSprites();
  if(carGroup.isTouching(person) && (gameState==="start" ||  gameState === "state2")){
   ...
   if(person.y === 350 && gameState === "stop"){
	   gameState = "stop2";
	   endLevel2();
	   gameOver = createSprite(750,350,1500,700);
	   gameOver.addImage(gameOver_img);
...
function spawnGirl(){
	girl = createSprite(random(1000,1300));
...
function endLevel3(){
	level_completed_sprite_level3.visible = true;
	if(mousePressedOver(level_completed_sprite_level3)){
		level_completed_sprite_level3.destroy();
